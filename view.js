...
function setupObserver(gsap) {
  ...
  containerView = document.querySelector('.messages-group-inner');
  if (!containerView) {
    console.warn('Could not find containerView .messages-group-inner');
    return;
  }
  console.log('containerView found:', containerView);

  observerView = new MutationObserver((mutations) => {
  ...
  observerView.observe(containerView, {
    childList: true,
    subtree: true
  });

  const existingMessages = containerView.querySelectorAll('.message-wrapper .messages-single');
...
function setupContainerObserver(gsap) {
  if (containerObserverView) {
    ...
    containerObserverView = null;
  }
  containerObserverView = new MutationObserver(() => {
    setupObserver(gsap);
  });
  ...
  containerObserverView.observe(body, {
    childList: true,
    subtree: true
  });
}
